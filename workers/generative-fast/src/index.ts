...
import { orchestrateTwoPhase } from './lib/orchestrator-twophase';
import { createCallbackSSEStream } from './lib/stream-handler';
import { persistAndPublish, DAClient } from './lib/da-client';

/**
 * FAST WORKER - Two-Phase Generation Experiment
 *
 * This worker uses a two-phase content generation approach:
 * - Phase 1 (Haiku): Generate hero block + page context (~2-3 sec)
 * - Phase 2 (Sonnet): Generate remaining blocks (~12-18 sec)
 *
 * User sees content in ~3 seconds instead of ~20-30 seconds.
 *
 * See: PLAN-performance-optimization.md
 */
...
function handleStream(request: Request, env: Env): Response {
  ...
  return createCallbackSSEStream(async (emit) => {
    ...
    try {
      // Run two-phase orchestration (hero first, then remaining blocks)
      const result = await orchestrateTwoPhase(query, slug, env, emit);
