...
function buildImageRequests(
  ...
  for (const block of content.blocks) {
    ...
    const imagePrompt = extractImagePrompt(block.content);
    if (!imagePrompt && !decision?.prompt) continue;
...
function buildHeroHTML(content: any, variant: string): string {
  ...
  return `
    <div class="hero${variant !== 'default' ? ` ${variant}` : ''}">
      <div>
        <picture>
          <img src="${placeholder}" alt="${content.headline}" data-gen-image="hero">
        </picture>
      </div>
      <div>
        <h1>${escapeHTML(content.headline)}</h1>
        ${content.subheadline ? `<p>${escapeHTML(content.subheadline)}</p>` : ''}
        ${content.ctaText ? `<p><a href="${escapeHTML(content.ctaUrl || '#')}" class="button">${escapeHTML(content.ctaText)}</a></p>` : ''}
      </div>
    </div>
  `.trim();
}
  ...
  const cardsHtml = content.cards.map((card: any, i: number) => {
    ...
    return `
      <div>
        <div>
          <picture>
            <img src="${placeholder}" alt="${escapeHTML(card.title)}" data-gen-image="card-${i}">
          </picture>
        </div>
        <div>
...
function buildColumnsHTML(content: any, variant: string): string {
  const columnsHtml = content.columns.map((col: any, i: number) => {
    ...
    if (col.imagePrompt) {
      ...
      colContent += `
        <picture>
          <img src="${placeholder}" alt="${escapeHTML(col.headline || '')}" data-gen-image="col-${i}">
        </picture>
      `;
    }
